  ...
  return (
    <>
          ...
          {galleryArr?.map((element, index) => {
            ...
            return (
              <div key={index}>
                ...
                {/* <Image src="/village1.jpg" className="absolute block max-w-full h-auto -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" height={1000} width={1000} alt="=Village image" /> */}
                <Image
                  src={`${process.env.URL}${element.attributes.url}`}
      ...
      <h2>
        ...
        ---------{sub_district?.data?.attributes?.name}
      </h2>
      <h1>
        <b>Notices</b>
      </h1>
      <div className="flex flex-row flex-wrap border-indigo-600">
        {noticeArr?.map((element, index) => {
          return (
            <div
              key={index}
              className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 w-80"
            >
      ...
      <div className="flex flex-row flex-wrap border-indigo-600">
        {authorityArr?.map(async (element, index) => {
          // console.log({element});
          const url = `${process.env.URL}/api/authorities/${element.id}?populate=*`;
          const token = process.env.AUTH_TOKEN;
          const requestOptions = {
            method: "GET",
            headers: { Authorization: `Bearer ${token}` },
          ...
          const data = await res.json();
          // console.log(data);
          const member = data.data.attributes.member;
          // console.log({member});
          ...
          return (
            ...
            >
              <div className="p-5">
                ...
                <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{member.data.attributes.firstname}  {member.data.attributes.lastname}</p>
                <Link
                  href={{pathname:`authority/${member.data.id}`,query: { name: `${element.attributes.designation}`}}}
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                >
                  Read more
                  <svg
